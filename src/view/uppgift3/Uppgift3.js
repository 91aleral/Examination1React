...
export const Uppgift3 = () => {
	...
	const getCharacterNameFromStarwarsAPI = async () => {
		const { data } = await StarwarsService.getStarwarsCharacter(currentCount)
		setCurrentCharacter({ data }.data)
	}

	const buttons = () => {
		return <div>
			<button onClick={() => setCurrentCount(currentCount +1)}>Increment</button> <br />
			<button onClick={() => setCurrentCount(currentCount -1)}>Decrement</button>
		</div>
	}
	...
	const displayCharacterName = () => {
		if (currentCharacter || undefined) {
			return <div>
				<h2>{(currentCharacter || undefined)?.name}</h2>
			</div>
		}
	}
